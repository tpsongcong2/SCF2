  ...
  const printTrip=trip=>{
    ...
    const rows=tripOrders.map(o=>{
      ...
      const lineRows=(o.lines||[]).map(l=>{
        ...
        const bold=isPiece(l)||brick,className='product-line'+(brick?' product-brick':yellow?' product-yellow':'');
      ...
      const items=lineRows.map(line=>'<div class="'+line.className+'">'+line.product+'</div>').join('');
      const quantities=lineRows.map(line=>'<div class="'+line.className+' qty-line">'+line.quantity+'</div>').join('');
      const delivered=lineRows.map(line=>'<div class="'+line.className+'"><span class="delivery-blank"></span></div>').join('');
      return '<tr><td class="order-middle">'+(deliveryOrderValue(o)||'')+'</td><td class="order-middle">'+(o.pointName||o.customer||'')+'</td><td class="product-cell">'+items+'</td><td class="product-cell" style="text-align:right;font-weight:700">'+quantities+'</td><td class="product-cell">'+delivered+'</td><td class="order-middle">'+(o.deliveryTime||'')+'</td></tr>';
    }).join('');
    const printHtml='<html><head><meta charset="UTF-8"><title>Chuyến '+trip.id+'</title><style>*{-webkit-print-color-adjust:exact;print-color-adjust:exact}body{font-family:Arial;padding:16px;font-size:13px}table{border-collapse:collapse;width:100%}th,td{border:1px solid #222;padding:5px 8px;vertical-align:top}th{background:#d9e8d9}.order-middle{vertical-align:middle;text-align:center}.product-cell{padding:0}.product-line{box-sizing:border-box;min-height:22px;line-height:20px;border-bottom:1px solid #222;padding:2px 8px;color:#000}.product-line:last-child{border-bottom:0}.product-brick{background:#d9795f;color:#000;font-weight:700}.product-yellow{background:#ffeb3b;color:#000}.qty-line{text-align:right}.delivery-blank{display:inline-block;width:70px;height:18px}.total{font-weight:700;text-align:right;padding:8px;background:#f5fbf5}.pack-summary{margin-top:16px;page-break-inside:avoid}.pack-summary h3{margin:0 0 7px;color:#2d6a4f;text-transform:uppercase;font-size:15px}.pack-summary th{background:#fff2b8}.pack-summary .num{text-align:right;white-space:nowrap}.summary-row td{background:#f5fbf5}.pack-note{font-size:11px;margin-top:6px;font-style:italic}@media print{@page{size:A4 landscape;margin:8mm}body{padding:0}.product-line{border-bottom-color:#000}.product-brick{background:#d9795f!important;color:#000!important}.product-yellow{background:#ffeb3b!important;color:#000!important}th,td{border-color:#000}}</style></head><body><h2>Chuyến giao hàng</h2><p>Ngày: <b>'+trip.deliveryDate+'</b> &nbsp;|&nbsp; Ca: <b>'+(trip.shiftName||'—')+'</b> &nbsp;|&nbsp; Lái xe: <b>'+(trip.driverName||'—')+'</b></p><table><thead><tr><th>STT</th><th>Địa điểm</th><th>Hàng hóa</th><th>SL đặt</th><th>SL giao</th><th>Giờ giao</th></tr></thead><tbody>'+rows+'</tbody></table><div class="total">Tổng: '+tripOrders.length+' đơn</div>'+packSummary+'<br><div style="display:flex;justify-content:space-around;margin-top:24px"><div style="text-align:center;width:40%"><div>Kho xuất hàng</div><div style="height:50px"></div><small>(Ký tên)</small></div><div style="text-align:center;width:40%"><div>Lái xe</div><div style="height:50px"></div><small>(Ký tên)</small></div></div></body></html>';
